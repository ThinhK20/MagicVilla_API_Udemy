...
namespace MagicVilla_VillaApi.Controllers
{
    ...
    public class VillaNumberAPIController : Controller
    {
        ...
        public async Task<ActionResult<APIResponse>> GetVillaNumbers()
        {
            try
            {
                var villaNumbers = await _db.GetAllAsync();
        ...
        public async Task<ActionResult<APIResponse>> DeleteVillaNumber([FromRoute] int villaNo)
        {
            try
            {
                ...
                if (villaNo <= 0)
                {
                    _response.StatusCode = HttpStatusCode.BadRequest;
                    _response.IsSuccess = false;
                    ModelState.AddModelError("CustomError", "Villa No is not valid !");
                    return BadRequest(_response);
                }
                var villaNumber = await _db.GetAsync(x => x.VillaNo == villaNo);
                if (villaNumber == null)
                {
                    _response.StatusCode = HttpStatusCode.NotFound;
                    _response.IsSuccess = false;
                    ModelState.AddModelError("CustomError", "Villa number is not exists !");
                    return NotFound(_response);
                }
